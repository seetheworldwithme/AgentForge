...
- **RAG 知识库**：文档入库（PDF / DOCX / XLSX 等）、分层语义切分（代码块/表格保护、中文 rune 安全）、**父子分块**（小块召回、大块返回上下文）、内容去重（sha256）、可选 **QA 索引 / 摘要索引**（LLM 生成）、**query 改写扩展 + 混合检索**（sqlite-vec 向量 + FTS5 trigram，跨 query RRF 融合）、可选 **rerank 重排**（Jina/Cohere 兼容）、**embedding 缓存**（省重复嵌入），对话时按相似度阈值注入，低质量片段自动过滤。
