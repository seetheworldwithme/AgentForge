...
- **RAG 知识库**：文档入库（PDF / DOCX / XLSX 等）、**文档图片转描述**（由 VLM 转录图片中的文字与示意图；发送前自动压缩限尺寸 + 单图超时，描述结果按 (模型,图片哈希) 缓存，慢模型/大文档跨次入库不重复描述、可断点收敛）、分层语义切分（代码块/表格保护、中文 rune 安全）、**父子分块**（小块召回、大块返回上下文）、内容去重（sha256）、可选 **QA 索引 / 摘要索引**（LLM 生成）、**query 改写扩展 + 混合检索**（sqlite-vec 向量 + FTS5 trigram，跨 query RRF 融合）、可选 **rerank 重排**（Jina/Cohere 兼容）、**embedding 缓存**（省重复嵌入）、**入库实时进度 + 断点续传**（core 重启自动恢复，可手动暂停/继续），对话时按相似度阈值注入，低质量片段自动过滤。
...
- **图片预览**：发送前后的图片缩略图均可点击放大查看，支持多图左右翻页（←/→）、下载与 Esc 关闭。
